...
cover: "https://cdn.jsdelivr.net/gh/madneal/blog-image@main/images/optimized/covers/a0b00672828225019d89.webp"
...
1. 从 `main`/`Execute` 看清 CLI 如何分发子命令  
2. 找核心接口（如 `PreRun`/`Run`）理解扩展点  
3. 对照「目录爆破」数据流：字典 → 并发 worker → HTTP → 结果过滤  
4. 记录可复用设计：限速、上下文取消、输出格式  

## 对自己工具的启发

写扫描器时优先：上下文取消、错误分类、可测试的 HTTP 客户端接口。gobuster 流行不只因为快，更因为结构清晰、模块可插拔。

## 小结

读工具源码是成为「能造轮子」的捷径。入口篇先建立地图，再钻 dir 模块细节。
