...
cover: "https://cdn.jsdelivr.net/gh/madneal/blog-image@main/images/optimized/covers/05756dd92f63d88d4f84.webp"
...
1. **存储前**：服务端 HTML 消毒（白名单标签/属性）  
2. **输出时**：正确上下文编码；CSP 降低危害  
3. **前端**：`js-xss` 等仅作辅助，不可信为唯一防线  
4. **编辑器**：选维护活跃、有安全说明的组件  

## 绕过意识

事件属性、协议、SVG/MathML、编码嵌套、模板二次渲染，都是富文本 XSS 的老舞台。回归测试应包含已知绕过向量库。

## 小结

富文本不能「一刀转义」，只能 **白名单消毒 + 多层防御**。把 XSS 当产品功能缺陷管理，而不是扫出来再补。
