...
*Figure 2: Fix commit from golang/go fixing a nil panic in its net package (PR #60823). The nil panic is caused by calling method String() on the return of RemoteAddr() on L1859, which can be a nil interface value (as shown in Figure 3)（原图链接已失效且无法恢复，已移除）*
...
*Figure 3: Excerpt from net/net.go showing the net.Conn interface and the implementation of the RemoteAddr() method by the struct net.conn, which can return nil, if c.ok() is false（原图链接已失效且无法恢复，已移除）*
...
*Figure 4: Excerpt from the implication graph built by NilAway representing the nil flow for the example in Figure 2.（原图链接已失效且无法恢复，已移除）*
...
*Figure 5: Error message reported by NilAway for the unsafe flow in Figure 2（原图链接已失效且无法恢复，已移除）*
...
*Figure 6: Architecture of NilAway.（原图链接已失效且无法恢复，已移除）*
...
*Figure 7: Simplified and redacted code excerpt from an internal Uber service logging 3000+ nil panics per day in production.（原图链接已失效且无法恢复，已移除）*
...
*Figure 8: Complete addressal of the 3000+ nil panics being logged per day in production.（原图链接已失效且无法恢复，已移除）*
