...
到目前为止，我们已经讨论过同源策略如何只允许我们发送请求，而不能读取响应。作为一个侧面步骤，让我们看看一个边缘情况，即使在完全SOP生效的情况下，我们也可以实际读取响应 - 也就是说，没有CORS可以放松SOP施加的限制。阅读回复的能力基本上允许我们使用受害者的浏览器作为代理从外部冲浪受害者的内部网络。

SOP不会阻止从同一来源的页面读取，并且页面可能会更改其自己的原点，以便能够与子域\ [21 \]进行通信。例如，如果http://targetorg.com已配置为能够与http://intranet.targetorg.com对话，那么如果我们能够在targetorg上运行JavaScript代码，我们可以像攻击者一样。 COM。 http://targetorg.com上的跨站点脚本（XSS）漏洞可能允许此方案。

在用于我们设置的实验室环境中，我们将DNS条目targetorg.com指向公共IP地址，因此，它可供攻击者（即我们）进行试验。攻击者在http://targetorg.com网站上发现了一个XSS漏洞：
