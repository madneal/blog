...
* 向内网大量不同主机的发出连接（对于某些常见的 HTTP 端口，如 80, 443, 8080）之后与外部Web服务器的连接可能表示恶意 JavaScript 在进行端口扫描。
* 连接到外部Web服务器，然后向内部网络发送大量传出HTTP请求，以响应HTTP / 404，这可能表示在开放端口上发现了恶意JavaScript指纹识别服务。
